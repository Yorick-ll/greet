...
func (m *defaultBlockModel) FindFirstFailedBlock(ctx context.Context) (*Block, error) {
	var resp Block
...
func (m *defaultBlockModel) FindProcessingSlots(ctx context.Context, slot int64, limit int) ([]*Block, error) {
	var resp []*Block
	err := m.conn.WithContext(ctx).Where("status = ? and slot >=?", constants.BlockFailed, slot).
		Order("slot desc").
		Limit(limit).
		Find(&resp).Error
